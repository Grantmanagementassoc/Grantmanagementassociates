...
export function TeamGridClient({ team }: { team: TeamMember[] }) {
  ...
  return (
    <>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 w-full max-w-7xl mx-auto mt-12">
        {team.map((m, idx) => (
          <ScrollReveal key={idx}>
            <div 
              className="group relative flex flex-col rounded-xl overflow-hidden bg-[#0a0a0a] border border-glass-border hover:border-cyan-500/50 transition-all cursor-pointer shadow-lg hover:shadow-cyan-900/20 h-[420px]"
            ...
            >
              ...
              <div className="h-[100px] p-4 bg-black flex flex-col items-start justify-center">
                <h3 className="text-lg font-bold text-white line-clamp-1">{m.name}</h3>
                <p className="text-sm text-cyan-400 mt-1 line-clamp-2">{m.role}</p>
              </div>
            </div>
          </ScrollReveal>
        ))}
