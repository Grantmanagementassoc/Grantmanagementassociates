...
export default async function CaseStudyDetail({ params }: { params: Promise<Params> }) {
  ...
  return (
    <>
          ...
          <div className="grid md:grid-cols-[1.4fr_1fr] gap-10 items-end">
            <div>
              <div className="flex flex-wrap gap-2">
                <span className="text-[11px] uppercase tracking-widest px-2.5 py-1 rounded-full border border-white/10 bg-white/5 text-slate-200">{cs.type}</span>
                <span className="text-[11px] uppercase tracking-widest px-2.5 py-1 rounded-full border border-white/10 bg-white/5 text-slate-200">{cs.year}</span>
                {ind && <Link href={`/industries/${ind.slug}`} className="text-[11px] uppercase tracking-widest px-2.5 py-1 rounded-full border border-white/10 bg-white/5 text-cyan-300 hover:bg-white/10">{ind.name}</Link>}
              </div>
              <h1 className="mt-6 text-4xl md:text-6xl font-semibold text-white leading-[1.02]">{cs.client}</h1>
              <p className="mt-4 text-lg text-slate-300">{cs.agency}</p>
            </div>
            <GlassCard hover={false} className="text-center md:text-right">
              <div className="text-xs uppercase tracking-widest text-slate-400">Awarded</div>
      ...
      <Section>
        <div className="grid md:grid-cols-3 gap-6">
          <GlassCard hover={false}>
            ...
            <p className="mt-4 text-slate-300 leading-relaxed">{cs.challenge}</p>
          </GlassCard>
          <GlassCard hover={false}>
            <Eyebrow>Approach</Eyebrow>
            <p className="mt-4 text-slate-300 leading-relaxed">{cs.approach}</p>
          </GlassCard>
          <GlassCard hover={false}>
            <Eyebrow>Result</Eyebrow>
            <p className="mt-4 text-slate-300 leading-relaxed">{cs.result}</p>
          </GlassCard>
        </div>
      </Section>
      ...
      {cs.quote && (
        <Section>
          <div className="glass rounded-3xl p-10 md:p-14 max-w-4xl mx-auto text-center">
            ...
            <p className="mt-4 text-2xl md:text-3xl text-white leading-snug font-display">{cs.quote.text}</p>
            <div className="mt-6 text-sm text-slate-400">— {cs.quote.author}, {cs.quote.role}</div>
          </div>
        </Section>
      )}
      ...
      {related.length > 0 && (
        <Section>
          <h2 className="text-2xl font-semibold text-white mb-6">Related case studies</h2>
              ...
              <Link key={r.slug} href={`/case-studies/${r.slug}`} className="group">
                <GlassCard>
                  <div className="flex justify-between">
                    <div>
                      <div className="text-xs uppercase tracking-widest text-slate-400">{r.year}</div>
                      <h3 className="mt-2 text-lg font-semibold text-white group-hover:text-cyan-300">{r.client}</h3>
                    </div>
