...
export function SiteFooter() {
  ...
  return (
    <footer className="relative mt-32 border-t border-white/5 bg-gradient-to-b from-transparent to-black">
      ...
      <div className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_repeat(4,1fr)]">
          <div>
            ...
            <p className="mt-4 text-sm text-slate-400 leading-relaxed max-w-sm">
              ...
              <span className="text-white font-medium">{site.totalSecured}</span> secured for organizations at every scale.
            </p>
            <div className="mt-6">
              <NewsletterForm />
            </div>
            <div className="mt-6 flex flex-wrap gap-2">
              {site.certifications.map((c) => (
                <span key={c} className="text-[10px] uppercase tracking-widest text-slate-400 border border-white/10 rounded-full px-3 py-1">
                  {c}
                </span>
              ))}
            </div>
          ...
          {footerNav.map((col) => (
            <div key={col.title}>
              <h3 className="text-xs uppercase tracking-widest text-slate-400 mb-4">{col.title}</h3>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-4">
            <a href={site.social.linkedin} aria-label="LinkedIn" className="text-slate-400 hover:text-white transition-colors">
            ...
            <a href={site.social.twitter} aria-label="Twitter" className="text-slate-400 hover:text-white transition-colors">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
            <a href={site.social.facebook} aria-label="Facebook" className="text-slate-400 hover:text-white transition-colors">
